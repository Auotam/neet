...
type Props = {
  ...
  searchParams: Promise<{ guided?: string; interaction?: string }>;
};
...
export default async function ExamPage(props: Props) {
  const { slug } = await props.params;
  const { guided, interaction } = await props.searchParams;
  const paper = getExamBySlug(slug);
  if (!paper) notFound();
  ...
  return (
    <div className="min-h-full bg-zinc-50 dark:bg-black">
      ...
      <footer className="mx-auto max-w-6xl px-4 pb-10 text-center text-xs text-zinc-500 dark:text-zinc-500">
